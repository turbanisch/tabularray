#' documentation
#' type: The type of LaTeX table environment. This can be either "float" (the default), "simple", or "break". The corresponding LaTeX environments defined in the **tabularray** package are "talltblr", "tblr", and "longtblr". "float" produces a floating numbered table that accommodates both a title and table notes, similar to a "threeparttable" in LaTeX. "simple" produces an unnumbered and untitled table intended for simple tabular matter to appear in the run of the text (see CMOS 3.51). "break" produces a numbered table that can be broken across pages instead of floating. It accommodates both a title and table notes, similar to a "longtable" in LaTeX. Could use table: float + simple + break ~ tabularray environment + traditional LaTeX environment + Description
