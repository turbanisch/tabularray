...
The popular table packages [gt][2], [kableExtra][3], and [flextable][4] do not
target the [tabularray][1] LaTeX package. The one that does is [tinytable][5] — a
feature-complete, actively developed package that offers several output formats
and can address `tabularray`'s inner and outer specifications directly. This
package predates `tinytable` and is, for most purposes, superseded by it.

`tabularray` may still hit a sweet spot if you want a small tool tailor-made for
`tabularray` LaTeX output, with a tidyverse- and gt-inspired interface for the
most common formatting tasks.
